...
void CUrlRequest::post(string url, const string &json, function<void(const char*body, size_t len)> func_finish, function<void(const int code)> failt_callback) {
	this->post(url, Headers(), json, func_finish, failt_callback);
}

void CUrlRequest::post(string url, const Headers &headers, const string &json, function<void(const char*body, size_t len)> func_finish, function<void(const int code)> failt_callback) {
	vector<char> data;
	CURL * curl = _pCUrl.get();
	if (curl != NULL) {
		...
		curl_easy_setopt(curl, CURLOPT_URL, url.c_str());
		curl_easy_setopt(curl, CURLOPT_POSTFIELDS, json.c_str()); // 设置要POST的JSON数据  
		curl_easy_setopt(curl, CURLOPT_HTTPHEADER, headerlist);
		curl_easy_setopt(curl, CURLOPT_WRITEDATA, &data);//设置CURLOPT_WRITEFUNCTION的最后一个参数值
		...
		if (_res != CURLE_OK) {
			...
			//TRACE(L"%S\n", to_wchar_t(curl_easy_strerror(_res)));
